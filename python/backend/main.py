"""
GoNote - Self-Hosted Markdown Knowledge Base
...
# Mount static files
static_path = Path(__file__).parent.parent.parent / "shared" / "frontend"
...
async def login_page(request: Request, error: str = None):
    ...
    content = content.replace('GoNote', app_name)
...
async def list_themes():
    ...
    themes_dir = Path(__file__).parent.parent.parent / "shared" / "themes"
...
async def catch_all(full_path: str, request: Request):
    ...
    async with aiofiles.open(index_path, 'r', encoding='utf-8') as f:
        content = await f.read()
    app_name = config['app']['name']
    return content.replace('<title>GoNote</title>', f'<title>{app_name}</title>')
